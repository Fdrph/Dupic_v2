...
static void SDL_Clay_RenderFillRoundedRect(Clay_SDL3RendererData *rendererData, const SDL_FRect rect, const float cornerRadius, const Clay_Color _color) {
    ...
    if (!vertices || !indices) { SDL_free(vertices); SDL_free(indices); return; }
...
static void SDL_Clay_RenderArc(Clay_SDL3RendererData *rendererData, const SDL_FPoint center, const float radius, const float startAngle, const float endAngle, const float thickness, const Clay_Color color) {
    ...
    for (float t = thicknessStep; t < thickness - thicknessStep; t += thicknessStep) {
        ...
        for (int i = 0; i <= numCircleSegments; i++) {
            const float angle = radStart + (float)i * angleStep;
            points[i] = SDL_FPoint{
    ...
    for (int i = 0; i < rcommands->length; i++) {
        ...
        switch (rcmd->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                TTF_Text *text = TTF_CreateText(rendererData->textEngine, font, config->stringContents.chars, (size_t)config->stringContents.length);
                if (text) {
                    ...
                    // Round to integer pixel positions to avoid sub-pixel blurring
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                //edges
                SDL_SetRenderDrawColor(rendererData->renderer, (Uint8)config->color.r, (Uint8)config->color.g, (Uint8)config->color.b, (Uint8)config->color.a);
                ...
                if (config->cornerRadius.topLeft > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, SDL_FPoint{centerX, centerY}, clampedRadii.topLeft,
                        180.0f, 270.0f, (float)config->width.top, config->color);
                }
                if (config->cornerRadius.topRight > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, SDL_FPoint{centerX, centerY}, clampedRadii.topRight,
                        270.0f, 360.0f, (float)config->width.top, config->color);
                }
                if (config->cornerRadius.bottomLeft > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, SDL_FPoint{centerX, centerY}, clampedRadii.bottomLeft,
                        90.0f, 180.0f, (float)config->width.bottom, config->color);
                }
                if (config->cornerRadius.bottomRight > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, SDL_FPoint{centerX, centerY}, clampedRadii.bottomRight,
                        0.0f, 90.0f, (float)config->width.bottom, config->color);
                }

            } break;
            case CLAY_RENDER_COMMAND_TYPE_SCISSOR_START: {
                SDL_Rect clipRect = { (int)bounding_box.x, (int)bounding_box.y, (int)bounding_box.width, (int)bounding_box.height };
                SDL_SetRenderClipRect(rendererData->renderer, &clipRect);
                break;
            }
